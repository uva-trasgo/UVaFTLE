...
int main(int argc, char *argv[])
{
	...
	size_t global_size = nPoints + (nPoints % block_size ? -nPoints % block_size : 0);
	OPENCL_CHECK(clEnqueueNDRangeKernel(queue, fpga_compute_gradient, 1, NULL, &global_size, &block_size, 0, NULL, &kernel_event));
