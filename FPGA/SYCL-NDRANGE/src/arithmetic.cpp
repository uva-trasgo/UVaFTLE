                                ...
                                sycl::buffer<int, 1> *b_nFacesPerPoint, sycl::buffer<int, 1> *b_facesPerPoint, sycl::buffer<double, 1> *b_log_sqrt, double T)
{
	...
	return q->submit([&](sycl::handler &h)
	                 {
		auto coords = b_coords->get_access<sycl::access::mode::read>(h);
		auto flowmap = b_flowmap->get_access<sycl::access::mode::read>(h);
		...
		auto facesPerPoint = b_facesPerPoint->get_access<sycl::access::mode::read>(h);
		auto log_sqrt = b_log_sqrt->get_access<sycl::access::mode::discard_write>(h);
		h.parallel_for<class ftle2D> (nd_range<1>(range<1>(static_cast<size_t>(nPoints)), range<1>(32)), [=](id<1> i)
        //[[intel::num_simd_work_items(16), sycl::reqd_work_group_size(1, 1, 128), intel::no_global_work_offset(1)]]
        {
			int ip = i[0];
			int nDim = 2; 
                                ...
                                sycl::buffer<int, 1> *b_nFacesPerPoint, sycl::buffer<int, 1> *b_facesPerPoint, sycl::buffer<double, 1> *b_log_sqrt, double T)
{
	...
	return q->submit([&](sycl::handler &h)
		...
		auto facesPerPoint = b_facesPerPoint->get_access<sycl::access::mode::read>(h);
		auto log_sqrt = b_log_sqrt->get_access<sycl::access::mode::discard_write>(h);
		h.parallel_for<class ftle3D> (nd_range<1>(range<1>(static_cast<size_t>(nPoints)), range<1>(32)), [=](id<1> i)
        //[[intel::num_simd_work_items(16), sycl::reqd_work_group_size(1, 1, 128), intel::no_global_work_offset(1)]]
        {
			int ip = i[0];
			int nDim = 3; 
