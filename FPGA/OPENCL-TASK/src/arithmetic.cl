...
kernel void fpga_compute_gradient_2D(int nPoints, int nVertsPerFace, global const double *restrict coords,
                                     global const double *restrict flowmap, global const int *restrict faces,
                                     global const int *restrict nFacesPerPoint, global const int *restrict facesPerPoint,
                                     global double *restrict log_sqrt,
                                     double T)
{
	int nDim = 2;
	...
	for (int ip = 0; ip < nPoints; ip++)
	{
		...
		if (count == 4)
		{
			/* NOTE: be careful with denom_x and denom_y == zero */
...
__attribute__((max_global_work_dim(0)))
kernel void fpga_compute_gradient_3D(int nPoints, int nVertsPerFace, global const double *restrict coords,
                                     global const double *restrict flowmap, global const int *restrict faces,
                                     global const int *restrict nFacesPerPoint, global const int *restrict facesPerPoint,
                                     global double *restrict log_sqrt,
                                     double T)
	...
	for (int ip = 0; ip < nPoints; ip++)
	{
		...
		for (iface = 0; (iface < nFaces) && (count < 6); iface++)
		{
			...
			for (ivert = 0; (ivert < nVertsPerFace) && (count < 6); ivert++)
			{
				...
				{
					/* (i-1, j, k) */
					if ((coords[ivertex * nDim + 1] == coords[ip * nDim + 1]) && (coords[ivertex * nDim + 2] == coords[ip * nDim + 2]) && (coords[ivertex * nDim] < coords[ip * nDim]))
					{
						if (closest_points_0 == -1)
						{
							closest_points_0 = ivertex;
					...
					/* (i+1, j, k) */
					else if ((coords[ivertex * nDim + 1] == coords[ip * nDim + 1]) && (coords[ivertex * nDim + 2] == coords[ip * nDim + 2]) && (coords[ivertex * nDim] > coords[ip * nDim]))
					{
						if (closest_points_1 == -1)
						{
							closest_points_1 = ivertex;
					...
					/* (i, j-1, k) */
					else if ((coords[ivertex * nDim] == coords[ip * nDim]) && (coords[ivertex * nDim + 2] == coords[ip * nDim + 2]) && (coords[ivertex * nDim + 1] < coords[ip * nDim + 1]))
					{
						if (closest_points_2 == -1)
						{
							closest_points_2 = ivertex;
					...
					/* (i, j, k-1) */
					else if ((coords[ivertex * nDim] == coords[ip * nDim]) && (coords[ivertex * nDim + 1] == coords[ip * nDim + 1]) && (coords[ivertex * nDim + 2] < coords[ip * nDim + 2]))
					{
