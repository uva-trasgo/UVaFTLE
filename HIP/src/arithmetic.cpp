...
__global__ void gpu_compute_gradient_2D ( int stride, int numCoords, int nVertsPerFace, double *coords, double *flowmap, int *faces, int *nFacesPerPoint, int *facesPerPoint, double *ftle_matrix, double *d_W_ei, double *d_logSqrt, double T) //double *gra1, double *gra2 )
{
	...
	double gra10, gra11, gra20, gra21;
        ...
        {
	    	idxface = (th_id == 0) ? facesPerPoint[iface] : facesPerPoint[nFacesPerPoint[th_id-1] + iface];
    	...
    	else
    	{
			if ( count == 3 )
			{
				...
				{
					denom_y = coords[ closest_points_3 * nDim + 1] - coords[ closest_points_2 * nDim + 1];
                    gra10 = 1; //flowmap [ th_id * nDim ];//??
					gra20 = ( flowmap[ closest_points_3 * nDim ]     - flowmap[ closest_points_2 * nDim ] )     / denom_y;
					gra11 = 1;// flowmap [ th_id * nDim +1];//??
	                gra21 = ( flowmap[ closest_points_3 * nDim + 1 ] - flowmap[ closest_points_2 * nDim + 1 ] ) / denom_y;
				}
			}
			else
...
__global__ void gpu_compute_gradient_3D ( int stride, int numCoords, int nVertsPerFace, double *coords, double *flowmap, int *faces, int *nFacesPerPoint, int *facesPerPoint, double *ftle_matrix, double*d_W_ei, double *d_logSqrt, double T) //double *gra1, double *gra2 )
{
	...
	double gra10, gra11, gra12, gra20, gra21, gra22, gra30, gra31, gra32;
		...
		else if(del<0)
		{
				...
				double _xt = acos(T);
				double xt  = _xt/3;
				x1         = (-b-2*sqrt((float) A)*cos(xt)) / (3*a);
				x2         = (-b+sqrt((float) A)*(cos(xt)+sqrt((float) 3)*sin(xt)))/(3*a);
				x3         = (-b+sqrt((float) A)*(cos(xt)-sqrt((float) 3)*sin(xt)))/(3*a);
		} 
...
__global__ void gpu_log_sqrt_2D(int numCoords, double T, double * d_w, double * d_r ){
    ...
    if (th_id < numCoords){ 
        int    nDim = 2; 
        double max = d_w[th_id*nDim];      
        
        if (d_w[th_id*nDim + 1] > max ) max = d_w[th_id*nDim + 1];
...
__global__ void gpu_log_sqrt_3D(int numCoords, double T, double * d_w, double * d_r){
    ...
    if (th_id < numCoords){ 
        ...
        if (d_w[th_id*nDim + 1] > max ) max = d_w[th_id*nDim + 1];
        if (d_w[th_id*nDim + 2] > max ) max = d_w[th_id*nDim + 2];
