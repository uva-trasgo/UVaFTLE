...
double max_eigen_2D ( double A10, double A11, double A20, double A21 )
{
	...
	double eig1 = (A21 + A10 + sq) / 2;
	double eig2 = (A21 + A10 - sq) / 2;
		...
		double A30, double A31, double A32 )
{
	...
	return max_solve_3rd_degree_eq ( a, b, c, d );
}

void compute_gradient_2D ( int ip, int nVertsPerFace, double *coords, double *flowmap, int *faces, int *nFacesPerPoint, int *facesPerPoint, double *ftle_matrix)
{
        int nDim = 2; 
...
void compute_gradient_3D ( int ip, int nVertsPerFace, double *coords, double *flowmap, int *faces, int *nFacesPerPoint, int *facesPerPoint, double *ftle_matrix)
{
        int nDim = 3; 
        int iface, nFaces, idxface, ivert;
        int closest_points_0 = -1;
	...
	gra21 = ftle_matrix[4];
        ...
        // Matrix mult 
        ftle_matrix[0]     = gra10 * gra10 + gra11 * gra11 + gra12 * gra12;
        ftle_matrix[1] = gra10 * gra20 + gra11 * gra21 + gra12 * gra22;
        ftle_matrix[2] = gra10 * gra30 + gra11 * gra31 + gra12 * gra32;
        ftle_matrix[3] = gra20 * gra10 + gra21 * gra11 + gra22 * gra12;
        ftle_matrix[4] = gra20 * gra20 + gra21 * gra21 + gra22 * gra22;
        ftle_matrix[5] = gra20 * gra30 + gra21 * gra31 + gra22 * gra32;
        ftle_matrix[6] = gra30 * gra10 + gra31 * gra11 + gra32 * gra12;
        ftle_matrix[7] = gra30 * gra20 + gra31 * gra21 + gra32 * gra22;
        ftle_matrix[8] = gra30 * gra30 + gra31 * gra31 + gra32 * gra32;
}

double log_sqrt ( double T, double eigen ){
	eigen = log(sqrt(eigen));
        return eigen / T;
}   
